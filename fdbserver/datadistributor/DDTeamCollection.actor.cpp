...
DDTeamCollection::DDTeamCollection(DDTeamCollectionInitParams const& params)
  : db(params.db), doBuildTeams(true), lastBuildTeamsFailed(false), teamBuilder(Void()), lock(params.lock),
    ...
    initialFailureReactionDelay(
        delayed(params.onHealthy, SERVER_KNOBS->INITIAL_FAILURE_REACTION_DELAY, TaskPriority::DataDistribution)),
    initializationDoneActor(logOnCompletion(params.onHealthy && initialFailureReactionDelay)), recruitingStream(0),
